...
public class MarkAdressedStrat extends CommandStrat {

    public MarkAdressedStrat(Main main) {
        super(main);
        this.setTitle("View Unresolved Requests / Mark Requests as Resolved");
    }

    public MarkAdressedStrat(String title, int entityID) {
     ...
     * @param title
     * @param ID
     * @return MarkAdressedStrat command
     */
    @Override
    public Command getCommand(String title, int ID) {
        return new MarkAdressedStrat(title, ID);
    }

    /**
     * Gets hashmap of all pending requests
     * @param entityID
     * @return hashmap
     */
    @Override
    public HashMap<String, Integer> getMap(int entityID) {
        return main.getRequestController().getAllPendingRequests();
    }
    ...
    public Optional<Command> execute(DynamicMenuOperatorHub dynamicMenuOperatorHub){
        dynamicMenuOperatorHub.runMarkAddressed();
        return Optional.empty();
    }
}
